/* 
本文件存放必要的一些数据结构 用于封装检查重复数字的方法
1 ConflictListsModel  用冲突列表记录冲突单元格的模型
1.1 ConflictLists  ConflictListsModel的具体实现

2 numsCountModel	记录同行、列、块单元填入某个数字的个数
...
function ValuesCountModel (sudoku) {
	...
	for (let i=0; i<81; i++) {
		let unitData = {
			okValuesCount: 9,																	//可填入的数字个数
			unitValueCount: [0, 0, 0, 0, 0, 0, 0, 0, 0],										//相关单元格1~9已经填入的数字
		}
...
//主要方法，更新单元
ValuesCountModel.prototype.updateUnitData = function (
												index,			//待更新单元
												oldValue = -1	//旧值
												) {
	
	this.updateData(index, oldValue)

}

//更新冲突方法
ValuesCountModel.prototype.updateData = function (
														index,
														oldValue = -1
															) 
{

	let [, , ids] = this.sudoku.getRelatedUnitsId(index)	//计算所有相关单元号
	let value = this.sudoku.units[index].inputValue
	ids.push(index)
	
	for (let id of ids) {
		let unitI = this.sudoku.units[id]
		...
		if (unitIData.unitValueCount[unitI.inputValue-1] > 1 && unitI.inputValue > 0) {
			unitI.getConflictColor(this.getConflictColor())
			unitI.conflict()
		} else {
			unitI.deconflict()
		}
...
//计算冲突单元背景颜色
ValuesCountModel.prototype.getConflictColor = function() {
	let color = "red"
...
//提取单元数据
ValuesCountModel.prototype.getUnitData = function (index) {
...
ValuesCountModel.prototype.generateUnitValue = function (id) {
